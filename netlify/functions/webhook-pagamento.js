...
const PACOTES = [
  { msgs: 15000 }, { msgs: 30000 }, { msgs: 50000 }
];
exports.handler = async (event) => {
  ...
  try {
    ...
    const [clienteId, pacoteIndex] = (payment.external_reference || '').split('__');
    const pacote = PACOTES[parseInt(pacoteIndex)];
    if (!pacote || !clienteId) return { statusCode: 200, body: 'referencia invalida' };
    ...
    await fetch(`${SUPABASE_URL}/rest/v1/perfis?id=eq.${clienteId}`, { method: 'PATCH', headers: h, body: JSON.stringify({ creditos: atual + pacote.msgs }) });
    await fetch(`${SUPABASE_URL}/rest/v1/transacoes`, { method: 'POST', headers: h, body: JSON.stringify({ cliente_id: clienteId, quantidade: pacote.msgs, descricao: `MP #${paymentId}` }) });
